...
export class TableroDeDanoDeDocumentosComponent implements OnInit {
    ...
    obtenerPrestamosDeDocumentos(): void {
        ...
        this.prestamosDeDocumentosService
            .obtenerPrestamosDeDocumentos()
            .subscribe(
                (resp: any) => {
                    // Buscamos permisos
                    const opciones = this.menuService.opcionesPerfil.find(
                        (opcion: { cUrl: string }) =>
                            opcion.cUrl === "tablero-de-daño-de-documentos"
                    );
                            ...
                            for (const prestamos of resp) {
                                if (
                                    ...
                                    prestamosTemp.push({
                                        id: prestamos.id,
                                        dFechaSolicitud:
                                            prestamos.dFechaSolicitud,
                                        dFechaDevolucion:
                                            prestamos.dFechaDevolucion,
                                        dFechaDocEntregado:
                                            prestamos.dFechaDocEntregado,
                                        dFechaSolicitudT: this.datePipe.transform(
                                            prestamos.dFechaSolicitud,
                                            "dd-MM-yyyy"
                                        ),
                                        dFechaDevolucionT: this.datePipe.transform(
                                            prestamos.dFechaDevolucion,
                                            "dd-MM-yyyy"
                                        ),
                                        dFechaDocEntregadoT: this.datePipe.transform(
                                            prestamos.dFechaDevolucion,
                                            "dd-MM-yyyy"
                                        ),
                                        ...
                                        tHoraSolicitud: moment(
                                            prestamos.tHoraSolicitud,
                                            "h:mm"
                                        ).format("HH:mm"),
                                        tHoraDevolucion: moment(
                                            prestamos.tHoraDevolucion,
                                            "h:mm"
                                        ).format("HH:mm"),
                                        tHoraDocEntregado: moment(
                                            prestamos.tHoraDocEntregado,
                                            "h:mm"
                                        ).format("HH:mm"),
    ...
    filterDatatable(value): void {
        ...
        if (value.target.value === "") {
            ...
            const temp = this.prestamoDocumentos.filter(
                (d) =>
                    d.dFechaSolicitudT.toLowerCase().indexOf(val) !== -1 ||
                    !val ||
                    d.dFechaDevolucionT.toLowerCase().indexOf(val) !== -1 ||
                    !val ||
                    d.cSolicitante.toLowerCase().indexOf(val) !== -1 ||
                    !val ||
                    d.cTipoPrestamo.toLowerCase().indexOf(val) !== -1 ||
                    !val ||
                    d.cTipoExpediente.toLowerCase().indexOf(val) !== -1 ||
                    !val ||
                    d.tHoraSolicitud.toLowerCase().indexOf(val) !== -1 ||
                    !val ||
