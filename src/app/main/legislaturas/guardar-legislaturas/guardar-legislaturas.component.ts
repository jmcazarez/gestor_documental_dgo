...
export class GuardarLegislaturasComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.form.get("actual").valueChanges.subscribe((val) => {
            if (val) {
                if (val) {
    ...
    async guardar(): Promise<void> {
        ...
        this.legislatura.cLegislatura = this.form.get(
            "descripcionLegislatura"
        ).value;
        ...
        if (this.legislatura.id) {
            ...
            this.legislaturaService
                .actualizarLegislatura(this.legislatura)
                .subscribe(
                    (resp: any) => {
                        if (resp) {
                            this.spinner.hide();
                            Swal.fire(
                                "Éxito",
                                "Legislatura actualizada correctamente.",
                                "success"
                            );
            ...
            this.legislaturaService
                .guardarLegislatura(this.legislatura)
                .subscribe(
                    (resp: any) => {
                        if (resp) {
                            this.spinner.hide();
