...
export class GuardarMesaComponent implements OnInit {
    ...
    async ngOnInit(): Promise<void> {
        ...
        this.form = this.formBuilder.group({
            descripcion: [this.mesas.descripcion, [Validators.required, Validators.minLength(3), Validators.maxLength(100)]],
            legislatura: [{ value: this.arrLegislaturas }, [Validators.required]],
    ...
    async guardar(): Promise<void> {
        ...
        let secretario = this.detalles.find(meta => meta.cargo == 'Secretario')
        let secretario_auxiliar = this.detalles.find(meta => meta.cargo == 'Secretario Auxiliar')
        let vocals = this.detalles.find(meta => meta.cargo == 'Vocal')
        let vocal_auxiliars = this.detalles.find(meta => meta.cargo == 'Vocal Auxiliar')
        ...
        if (this.mesas.id) {
            ...
            if (this.mesas.detalle_participantes_mesa_directivas) {
                ...
                this.detalleMesaService.actualizarDetalleMesa({
                    id: this.mesas.detalle_participantes_mesa_directivas[0].id,
                ...
                this.detalleMesaService.guardarDetalleMesa({
                    id: this.mesas.detalle_participantes_mesa_directivas[0].id,
                    mesas_directiva: this.mesas.id,
                    presidentes: [presidente.idParticipante],
                    vicepresidentes: [vicepresidentes.idParticipante],
            ...
            // Actualizamos el mesa directiva
            this.mesasDirectivasService.actualizarMesa(this.mesas).subscribe((resp: any) => {
                if (resp) {
                    this.spinner.hide();
                    Swal.fire('Éxito', 'Mesa directiva actualizada correctamente.', 'success');
    ...
    async obtenerDetallesMesa(): Promise<void> {
        ...
        await this.detalleMesaService.obtenerDetalleMesa().subscribe((resp: any) => {
            ...
            if (this.optConsultar) {
                for (const detalleMesa of resp) {
                    ...
                    if (detalleMesa.mesas_directiva === this.mesas.id) {
                        ...
                        const partido = this.arrPartidos.find(meta => meta.id == detalleMesa.presidentes[0].partidos_politico);

                        if (detalleMesa.presidentes[0]) {
                            detallesMesasTemp.push({

                                cargo: 'Presidente',
                                idParticipante: detalleMesa.presidentes[0].id,
                                participante: detalleMesa.presidentes[0].nombre,
                        ...
                        if (detalleMesa.vicepresidentes[0]) {
                            detallesMesasTemp.push({
                                ...
                                idParticipante: detalleMesa.vicepresidentes[0].id,
                                participante: detalleMesa.vicepresidentes[0].nombre,
                                partido: partido.cNomenclatura,
                        ...
                        if (detalleMesa.secretario[0]) {
                            detallesMesasTemp.push({
                                ...
                                cargo: 'Secretario',
                                idParticipante: detalleMesa.secretario[0].id,
                                participante: detalleMesa.secretario[0].nombre,
                                partido: partido.cNomenclatura,
                        ...
                        if (detalleMesa.secretario_auxiliar[0]) {
                            detallesMesasTemp.push({
                                ...
                                idParticipante: detalleMesa.secretario_auxiliar[0].id,
                                participante: detalleMesa.secretario_auxiliar[0].nombre,
                                partido: partido.cNomenclatura,
                        ...
                        if (detalleMesa.vocal_auxiliars[0]) {
                            detallesMesasTemp.push({
                                ...
                                idParticipante: detalleMesa.vocal_auxiliars[0].id,
                                participante: detalleMesa.vocal_auxiliars[0].nombre,
                                partido: partido.cNomenclatura,
