...
export class MenuService {
                ...
                if (token !== undefined && token !== null) {
                    ...
                    await this.obtenerTipoOpciones(token).subscribe(async (resp: any) => {
                        ...
                        if (usuarioLogin) {
                                            ...
                                            if (documentos.tipo_de_documento.bActivo) {
                                                ...
                                                if (resultado) {
                                                    ...
                                                    if (documentos.tipo_de_documento.departamento) {
                                                        ...
                                                        cDepartamento = this.arrDepartamentos.find(dep => dep.id = departamento)
                                    ...
                                    for (const opciones of perfiles.Opciones) {
                                        ...
                                        if (opciones) {
                                            if (opciones.opciones_del_sistema) {
                                                /*   let sortedCompany = opciones.opciones_del_sistema.sort((a, b) => (a.nOrden < b.nOrden) ? -1 : 1);
                                                ...
                                                if (opciones.opciones_del_sistema.bActivo) {
                                                    ...
                                                    if (!resultado) {
                                                        ...
                                                        const itemMenu: ItemMenuModel = {
                                                            id: 'modulo-' + opciones.opciones_del_sistema.nOrden,
                            ...
                            let sortedRegistro = grupoMenu.children.sort((a, b) => (a.id < b.id) ? -1 : 1);
                            grupoMenu.children = sortedRegistro

                            let sortedReportes = grupoMenuReportes.children.sort((a, b) => (a.id < b.id) ? -1 : 1);
                            grupoMenuReportes.children = sortedReportes

                            let sortedConfiguracion = grupoMenuConfiguracion.children.sort((a, b) => (a.id < b.id) ? -1 : 1);
                            grupoMenuConfiguracion.children = sortedConfiguracion

                            let sortedCatalagos = grupoMenuCatalagos.children.sort((a, b) => (a.id < b.id) ? -1 : 1);
                            grupoMenuCatalagos.children = sortedCatalagos
    ...
    async obtenerPrivilegios(): Promise<any> {
        return new Promise(async (resolve) => {
...
/* 
                            ...
                            resolve({
                                ...
                                opcionesPerfil: this.opcionesPerfil,
                                tipoInformacion: this.tipoInformacion,
                                tipoOpciones: this.tipoOpciones,
                                arrDepartamentos: this.arrDepartamentos,
                            })
                            // }
                        }
                    }, err => {
